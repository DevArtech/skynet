"""
Train baseline MuZero on the competitive auction environment.

Usage:
    python train_auction_muzero.py --iterations 1000 --device cuda
"""
...
from auction_mcts import run_auction_mcts
from auction_model import AuctionMuZeroNet, build_default_auction_config
from auction_train import (
    BaselineReplayBuffer,
    BaselineTrainConfig,
    create_baseline_optimizer,
    generate_baseline_episode,
    train_baseline_step,
)
...
def _percentile(vals: list[float], q: float) -> float:
    ...
    if len(o) == 1:
        return o[0]
    p = max(0.0, min(1.0, q)) * (len(o) - 1)
    lo, hi = int(math.floor(p)), int(math.ceil(p))
    return o[lo] if lo == hi else (1 - (p - lo)) * o[lo] + (p - lo) * o[hi]


def _checkpoint_iter(p: Path) -> int:
    s = p.stem.replace("checkpoint_iter_", "")
    try:
        return int(s)
    except ValueError:
        return -1
...
def _build_history() -> dict[str, list]:
    return {
        ...
        "loss_value": [], "loss_reward": [], "grad_norm": [],
        "replay_steps": [], "selfplay_mean_steps": [],
        "selfplay_terminated_fraction": [], "selfplay_policy_entropy": [],
        "selfplay_root_value": [], "selfplay_num_simulations": [],
...
def _trim_history(h: dict, n: int) -> None:
    for k in h:
        h[k] = h[k][:n]
    row_count = len(h["iteration"])
    for k in h:
        while len(h[k]) < row_count:
...
def _save_graphs(h: dict, g: Path) -> None:
    ...
    specs = [
        ...
        ("eval_profit_diff_p0_minus_p1", "Self-Play Eval Profit Diff (P0-P1)", "Diff"),
...
def evaluate_model(
    model: AuctionMuZeroNet, episodes: int, sims: int, max_moves: int,
...
def evaluate_vs_bots(
    model: AuctionMuZeroNet, bot_names: list[str], episodes_per_bot: int,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train baseline MuZero on auction")
    ...
    parser.add_argument("--truncation-penalty", type=float, default=0.5)
    ...
    parser.add_argument("--output-dir", type=str, default="runs/auction_baseline")
    ...
    model = AuctionMuZeroNet(model_cfg).to(args.device)
    train_cfg = BaselineTrainConfig(
        ...
        truncation_penalty=args.truncation_penalty, device=args.device,
    )
    optimizer = create_baseline_optimizer(model, train_cfg)
    grad_scaler = torch.amp.GradScaler() if args.device == "cuda" else None
    replay = BaselineReplayBuffer(train_cfg.replay_capacity_episodes)
    opp_model = AuctionMuZeroNet(model_cfg).to(args.device)
    opp_model.eval()
    pool: list[dict] = []
    ...
    if args.resume or args.resume_checkpoint:
        ...
        print(f"[resume baseline] checkpoint={ckpt_path} iter={done}")
    ...
    if start_iter > args.iterations:
        print("[resume baseline] nothing to do")
        return
    ...
    for iteration in range(start_iter, args.iterations + 1):
        ...
        sp_cfg = MCTSConfig(
            num_simulations=sp_sims, discount=args.discount, temperature=1.0,
        ...
        for ep_idx in range(args.selfplay_episodes_per_iter):
            ...
            episode = generate_baseline_episode(
                model, sp_cfg, selected_opp, learner, env_factory,
                train_cfg.max_moves_per_episode, train_cfg.device,
                train_cfg.truncation_penalty, opp_selector,
            )
            replay.add_episode(episode)
            elapsed = time.perf_counter() - sp_start
            print(
                f"[selfplay baseline] iter={iteration:04d} ep={ep_idx + 1}/{args.selfplay_episodes_per_iter} "
        ...
        losses = {"loss_total": 0, "loss_policy": 0, "loss_value": 0, "loss_reward": 0, "grad_norm": 0}
        eff = 0
        for _ in range(args.train_steps_per_iter):
            if replay.total_steps() < train_cfg.batch_size:
                break
            sl = train_baseline_step(model, optimizer, replay, train_cfg, grad_scaler)
            for k in losses:
                losses[k] += sl[k]
            eff += 1
        if eff > 0:
        ...
        if iteration % args.eval_every == 0:
            print(f"[eval baseline] iter={iteration:04d}")
            ev.update(
                evaluate_model(
                    model,
                    args.eval_episodes,
            ...
            for bn in bot_names:
                bs = bot_det.get(bn, {})
                print(
                    f"[eval baseline][bot {bn}] games={bs.get('games_balanced', 0)} "
        ...
        print(
            f"[iter {iteration:04d}] loss={losses['loss_total']:.4f} "
            f"replay={replay.total_steps()} sims={sp_sims} "
